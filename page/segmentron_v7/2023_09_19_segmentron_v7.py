...
class segmentron:
    ...
    def __init__(self, segment_scoring_functions, accumulating_function, parameters):
        ...
        #Stores the GC count in each interval of length "overlap"
        self.parameters["gc_overlap_composition"] = []
        #Set to store the segmentation for future operations following every segmentation
        self.segmentation = []
    ...
    def segment(self, sequence, forbidden_regions, coarseness = 1, multiprocessing_cores = 0):
        ...
        search_sequence = self.parameters["sequence"]
        ...
        self.parameters["gc_overlap_composition"] = overlap_gc_counts
    ...
    def write_segments_to_bed(self, filepath):
        ...
        #Set up header
        lines = [f"#Chromosome Number\tStart\tEnd\tLabel\n"]
        for i in range(0, len(self.segmentation) - 1):
            #Write segments to the bed file
            lines.append(f"0\t{self.segmentation[i]}\t{self.segmentation[i + 1]}\tSegment {i + 1}\n")
        with open(filepath, "w") as f:
            for line in lines:
                f.write(line)
        return filepath
...
if __name__ == "__main__":
    segment_scoring_functions = [function_list.length_score, function_list.forbidden_region_score, function_list.overlap_composition, function_list.forbidden_region_class_score]
    parameters = {
                    "max_length" : 4663,
                    "min_length" : 1000,
                    "overlap" : 337
                }
    segmenter = segmentron(segment_scoring_functions, function_list.addition_function, parameters)
    ...
    total_score, segmentation = segmenter.segment_from_file(filepath, forbidden_region_classes = 1, multiprocessing_cores = 4, coarseness = 1)
    segmenter.print_results()
